...
class Graph {
 ...
 public:
  ...
  class Edge : private totally_ordered<Edge> {
   public:
    ...
    /** Return an index of a node of this Edge */
    size_type& index_1() {
      return n1_;
    }

    /** Return the other index of the other node Edge */
    size_type& index_2() {
      return n2_;
    }
    ...
    bool operator==(const Edge& e) const {
      ...
      if ((e.n1_ == this->n1_) && (e.n2_ == this->n2_) && (e.graph_ == this->graph_)) {
        return true;
      } else if ((e.n1_ == this->n2_) && (e.n2_ == this->n1_) && (e.graph_ == this->graph_)) {
        return true;
      }
    ...
    // Return length of the edge, euclidean distance between 2 nodes
    double length() const{
    ...
    edge_value_type& value() {
      auto it = find(graph_->edges_.begin(), graph_->edges_.end(), *this);
      int index = it - graph_->edges_.begin();
      return graph_->edge_values_[index];
    }
  ...
  /** Remove an adjacency from the graph
  * @pre @a root_ @a adjacent are valid indices of nodes of this graph
  * @post adjacency[ @a root_] does not contain @a adjacent
  *
  * Complexity: No more than O(num_nodes()+num_edges())
  */
  void remove_adjacency(const size_type root_, const size_type adjacent) {
    std::vector<size_type> vec_adj = adjacency.find(root_)->second;
    bool found = false;
    auto iter = vec_adj.begin();
    while (!found && (iter != vec_adj.end())) {
      if (*iter == adjacent) {
        found = true;
      } else {
        ++iter;
    ...
    if (found == true) {
      std::iter_swap(iter, vec_adj.end()-1);
      vec_adj.pop_back();
      adjacency.find(root_)->second = vec_adj;
    }
  ...
  * @return 1 if has_edge(n1, n2), 0 otherwise
  ...
  */
  size_type remove_edge(const Node& n1, const Node& n2) {
    ...
    if(has_edge(n1, n2)) {
      // remove edge from list of edges, edge values
      bool found = false;
      size_type ind = 0;
      
      while (!found && ind < edges_.size()) {
        if ((edges_[ind].node1() == n1) && (edges_[ind].node2() == n2)) {
          found = true;
        }
        else if ((edges_[ind].node1() == n2) && (edges_[ind].node2() == n1)) {
          found = true;
        } else {
          ind++;
        }
      ...
      if (found == true) {
        std::swap(edges_[ind], edges_[num_edges_ - 1]);
        edges_.pop_back();
        std::swap(edge_values_[ind], edge_values_[num_edges_ - 1]);
        edge_values_.pop_back();
      }

      // remove adjacency between two nodes
      remove_adjacency(n1.index(), n2.index());
      remove_adjacency(n2.index(), n1.index());
      
      // reduce number of edges by 1
  ...
  */
  size_type remove_edge(const Edge& edge) {
    size_type eval = remove_edge(edge.node1(), edge.node2());
    return eval;
  }
  ...
  */
  size_type remove_node(const Node& n) {
    ...
    if(has_node(n)) {
      // remove all edges between this node and others
      // remove this node's index from incident nodes' adjacency lists
      
      std::vector<size_type> adjacent_nodes = adjacency[n.index()];
      auto iter = adjacent_nodes.begin();
      while(iter != adjacent_nodes.end()) {
        remove_edge(n, nodes_[*iter]);
        ++iter;
        
      }

      // this node's index
      size_type this_ind = n.index();

      // remove this node's position, value, and node from graph data
      // replace with last node's position, value, and node.
      std::swap(nodes_[this_ind], nodes_[size_ - 1]);
      nodes_.pop_back();

      std::swap(points_[this_ind], points_[size_ - 1]);
      points_.pop_back();

      std::swap(node_values_[this_ind], node_values_[size_ - 1]);
      node_values_.pop_back();

      // change swapped node's index to this node's index
      nodes_[this_ind].index() = this_ind;

      // change swapped node's index in adjacency 
      std::vector<size_type> swapped = adjacency[size_- 1];
      adjacency[this_ind] = swapped;

      // remove swapped node's adjacency list
      adjacency.erase(size_ - 1);

      // change all references to swapped index
      for (auto map_iter = adjacency.begin(); map_iter != adjacency.end(); ++map_iter) {
        std::vector<size_type> curr_list = map_iter->second;
        for (size_type i = 0; i < curr_list.size(); i++) {
          if (curr_list[i] == (size_ - 1)) {
            curr_list[i] = this_ind;
            map_iter->second = curr_list;
          }
        }
      }

      //change all references to swapped index in edges
      for (auto edge_iter = edges_.begin(); edge_iter != edges_.end(); ++edge_iter) {
        Edge e = *edge_iter;
        if (e.index_1() == (size_ - 1)) {
          e.index_1() = this_ind;
        } else if (e.index_2() == (size_ - 1)) {
          e.index_2() = this_ind;
        }
      }
      size_ = size_ - 1;
 ...
 private:
  ...
  // corresponding vector for edge values
