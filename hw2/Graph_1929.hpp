...
class Graph {
 ...
 public:
  ...
  Graph() : nodes_(), edges_(), edge_values_(), edge_hash_(), size_(0), num_edges_(0) {
    // HW0: YOUR CODE HERE
  }
  ...
  class Edge : private totally_ordered<Edge>{
   public:
    ...
    edge_value_type& value(){
      return (graph_->edge_values_)[edge_idx_];
    }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    //Initialize values
    //--functionality_0
    //--edge value is not initialized for POD types (e.g. int)
    //--START
    edge_value_type v;
    //--END
    edge_values_.push_back(v);
  ...
  class IncidentIterator {
   public:
    ...
    IncidentIterator& operator++() {
      edge_idx += 1;
      if (edge_idx >= (graph->adjacency_map_[n1]).size()){
        edge_idx = (graph->adjacency_map_[n1]).size();
      }
      return *this;
    }
  ...
  //--functionality_1
  //--EdgeIterator failed test due to segfault
  //--END
  class EdgeIterator {
   public:
    // These type definitions let us use STL's iterator_traits.
   ...
   * @post If edge does not exist, graph unchanged. If edge exists, the
   * edge is deleted and number of edges is reduced by 1. Edge indices still range
   * from 0 <= edge_index < num_edges.
   * @return 1 if edge removed, 0 if not
   *
   * Nodes are not invalidated.
   * Edge indices are invalidated.
   * Edge iterators are invalidated, since data structures holding information
   * about the edges are modified.
   *
   * Time complexity is O(max_degree).
   */
  size_type remove_edge(const Edge& e){
    //Check if edge exists
    auto edges_from_node_1 = adjacency_map_[e.idx1_];
    if (std::count(edges_from_node_1.begin(),edges_from_node_1.end(),e.idx2_)==0){
      return 0;
    }
    //[Update edges_ and edge_values_]
    size_type idx_del = e.edge_idx_;
    size_type idx_last = num_edges_ - 1;
    std::swap(edges_[idx_del],edges_[idx_last]);
    std::swap(edge_values_[idx_del],edge_values_[idx_last]);
    edges_.pop_back();
    edge_values_.pop_back();
    // [Update edge_hash_]
    pair swapped_edge = edges_[idx_del];
    edge_hash_[swapped_edge] = idx_del; //Adjust index of swapped edge
    pair t = std::make_pair(e.idx1_,e.idx2_);
    pair t2 = std::make_pair(e.idx2_,e.idx1_);
    edge_hash_.erase(t); //Erase edge from edge_hash
    edge_hash_.erase(t2);
    // [Update adjacency_map]
    std::vector<size_type>::iterator it = std::find(adjacency_map_[e.idx1_].begin(),adjacency_map_[e.idx1_].end(),e.idx2_);
    adjacency_map_[e.idx1_].erase(it);
    std::vector<size_type>::iterator it2 = std::find(adjacency_map_[e.idx2_].begin(),adjacency_map_[e.idx2_].end(),e.idx1_);
    adjacency_map_[e.idx2_].erase(it2);
    // [Update num_edges_]
    num_edges_ -= 1;
    return 1;
  }
  ...
  size_type remove_edge(const Node& n1, const Node& n2){
    ...
    pair t = std::make_pair(n1.idx_,n2.idx_);
    pair t2 = std::make_pair(n2.idx_,n1.idx_);
    size_type e_idx;
    if (edge_hash_.count(t)>0){
      e_idx = edge_hash_[t];
    }
    ...
    Edge e = Edge(this, n1.idx_, n2.idx_, e_idx);
    return remove_edge(e);
  }

  /**
   * @brief Removes a given node.
   * @post If node does not exist, graph unchanged. If node exists, the
   * node is deleted and number of nodes is reduced by 1. Node indices still range
   * from 0 <= edge_index < num_nodes.
   * @return 1 if node removed, 0 if not
   *
   * Node indices are invalidated.
   * Edge indices are invalidated.
   * Node and edge iterators are invalidated.
   *
   * Time complexity is O(max_degree).
   */
  //--functionality_1
  //--node removal after position change failed test (segfault)
  //--START
  size_type remove_node (const Node& n){
    ...
    std::vector<size_type> edges = adjacency_map_[n.idx_];
    for (auto it = edges.begin(); it != edges.end(); ++it){
      size_type n2_idx = *it;
      Node n2 = node(n2_idx);
      remove_edge(n, n2);
    }
    size_type idx_del = n.idx_;
    size_type idx_last = size_ - 1;
    std::swap(nodes_[idx_del],nodes_[idx_last]);
    nodes_.pop_back();
    nodes_[idx_del].idx = idx_del;
    size_ -= 1;
    adjacency_map_.erase(idx_del);

    std::vector<size_type> adj_edges = adjacency_map_[idx_last];
    for (auto it2 = adj_edges.begin(); it2 != adj_edges.end(); ++it2){
      size_type adj_node = *it2;
      std::vector<size_type>::iterator it3 = std::find(adjacency_map_[adj_node].begin(),adjacency_map_[adj_node].end(),idx_last);
      adjacency_map_[adj_node].erase(it3);
      adjacency_map_[adj_node].push_back(idx_del);
      pair t = std::make_pair(idx_last,adj_node);
      pair t2 = std::make_pair(adj_node,idx_last);
      if (edge_hash_.count(t)>0){
        edge_hash_[std::make_pair(idx_del,adj_node)] = edge_hash_[t];
        edges_[edge_hash_[t]] = std::make_pair(idx_del,adj_node);
        edge_hash_.erase(t);
      }
      else {
        edge_hash_[std::make_pair(adj_node,idx_del)] = edge_hash_[t2];
        edges_[edge_hash_[t2]] = std::make_pair(adj_node,idx_del);
        edge_hash_.erase(t2);
      }
    }
    adjacency_map_[idx_del] = adjacency_map_[idx_last];
    adjacency_map_.erase(idx_last);
    return 1;
  }
  //--START

  /**
   * @brief Removes the node pointed to by given node iterator.
   * @post If node does not exist, graph unchanged. If node exists, the
   * node is deleted and number of nodes is reduced by 1. Node indices still range
   * from 0 <= edge_index < num_nodes.
   * @return Iterator pointing to the node at the index of the deleted node. If
   * deleted node is the last one, returns iterator pointing to end of nodes.
   *
   * Node indices are invalidated.
   * Edge indices are invalidated.
   * Node and edge iterators are invalidated.
  ...
  /**
   * @brief Removes the edge pointed to by the given edge iterator.
   * @post If edge does not exist, graph unchanged. If edge exists, the
   * edge is deleted and number of edges is reduced by 1. Edge indices still range
   * from 0 <= edge_index < num_edges.
   * @return Iterator pointing to the edge at the index of the deleted node. If
   * deleted edge is the last one, returns iterator pointing to edges end.
   *
   * Nodes are not invalidated.
   * Edge indices are invalidated.
   * Edge iterators are invalidated, since data structures holding information
  ...
  edge_iterator remove_edge(edge_iterator e_it){
    ...
    remove_edge(*e_it);
    if (e_it.incident_iter == (*e_it.node_iter).edge_end()){
      ++e_it;
    }
    else if (e_it.visited_nodes.find((*e_it).node2().index()) != e_it.visited_nodes.end()){
      ++e_it;
    }
    return e_it;
  }
