 ...
 public:
  ...
  class Node: private totally_ordered<Graph::Node> {
   public:
    ...
    /** Return a modifiable reference to the node's position */
    Point& position() {
      //assert(valid());
      return const_cast<Graph*>(graph_)->nodes_.at(uid_).position_;
    } 
...
//      assert(valid());
      return const_cast<Graph*>(graph_)->nodes_.at(uid_).value_;
    }
    ...
    */ 
    size_type degree() const {
      assert(valid());
      auto it = graph_->node_to_edge_.find(uid_);
...
//      assert(n.valid());
    ...
    std::vector<Edge> get_inc_edges(const Node& n) const {
      return graph_->node_to_edge_.at(n.uid_);
    }
   ...
   private:
    ...
    bool valid() const {
      return uid_ >= 0 && uid_ < graph_->nodes_.size()           // uid in range
             && graph_->nodes_[uid_].idx_ < graph_->i2u_.size()  // idx in range
             && graph_->i2u_[graph_->nodes_[uid_].idx_] == uid_; // uid in sync
    }
  ...
  Node add_node(const Point& position,
    ...
    nodes_.emplace_back(position, value, idx);
    i2u_.push_back(uid);
...
//    }
   ...
   private:
    ...
    Edge(size_type index, std::tuple<Node, Node> node_pair, edge_value_type value): 
                                              index_(index),
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    node_to_edge_[a.uid_].push_back(edge);
    node_to_edge_[b.uid_].push_back(edge);
    edges_.emplace(edge);
    return edge;
  }
  ...
  Edge add_edge(const Node& a, const Node& b, edge_value_type value) {
    // check if edge is in the graph
    Edge edge(num_edges(), std::make_tuple(a,b), value);
    auto it = edges_.find(edge);
    // if it is in the graph
    if (it != edges_.end()) {return *it;}
    // else add the edge to the graph and return it
  ...
  class IncidentIterator {
   public:
    ...
    Edge operator*() const {
      ...
      Edge edge = graph_->node_to_edge_.at(node_.uid_).at(i_);
      Node node1 = std::get<0>(edge.node_pair_);
      if (node_ == node1) {return edge;}
      return Edge(edge.index_, std::make_tuple(node_, node1), edge.value_);
    }
  ...
  // Assuming sparse graph, should take at most O(num_nodes), hopefully a lot less
  size_type remove_node (const Node& n) { 
    ...
    // first check if there are edges associated with this node
    if (node_to_edge_.find(n.uid_) != node_to_edge_.end()) {
      // if so remove all the edges
      auto edges = node_to_edge_.at(n.uid_);
      if (edges.size() != 0) {
        for (auto it = node_to_edge_.at(n.uid_).begin(); it != node_to_edge_.at(n.uid_).end(); ++it) {
          remove_edge(*it);
        }    
      }
    }
    ...
    std::iter_swap(i2u_.begin() + idx, i2u_.end() - 1);
    i2u_.pop_back();
    nodes_.at(i2u_[idx]).idx_ = idx;

    return 1;
  }
  
  // Assuming sparse graph, should take at most O(num_nodes), hopefully a lot less
  // returns a valid iterator with a reference different from the one you have just deleted
  node_iterator remove_node ( node_iterator n_it ) {
    remove_node(*n_it);
    return node_begin();
  }
  
  // Assuming sparse graph, should take at most O(num_nodes+num_edges), hopefully a lot less
  size_type remove_edge (const Node& a, const Node& b) {
    // returns 1 if an edge is removed, 0 otherwise
    Edge edge(0, std::make_tuple(a,b));
    auto it = edges_.find(edge);
    if (it == edges_.end()) {
      return 0;
    }
    ...
    auto edges_a = node_to_edge_[a.uid_];
    auto it_a =std::find(edges_a.begin(), edges_a.end(), edge);
    node_to_edge_[a.uid_].erase(it_a);

    auto edges_b = node_to_edge_[b.uid_];
    auto it_b =std::find(edges_b.begin(), edges_b.end(), edge);
    node_to_edge_[b.uid_].erase(it_b);
  ...
  size_type remove_edge (const Edge& e){
    // returns 1 if an edge is removed, 0 otherwise
    auto node_pair = e.node_pair_;
    return remove_edge(std::get<0>(node_pair), std::get<1>(node_pair));
  }

  // Assuming sparse graph, should take at most O(num_nodes+num_edges), hopefully a lot less
  // returns a valid iterator with a reference different from the one you have just deleted
  edge_iterator remove_edge (edge_iterator e_it) {
    remove_edge(*e_it); // I could make it faster by passing e_it.it_ directly to erase()
    return edge_begin();  
  }
 ...
 private:
  ...
  // stores the info of all node that has ever been added to the graph, even if later removed
  std::vector<internal_node> nodes_;  // indexed by node uid

  // stores the currently active set of nodes
  std::vector<size_type> i2u_;        // indexed by node idx

  // stores the current set of all active edges
  std::set<Edge> edges_;

  // map from node uid to edge
  std::map<int, std::vector<Edge>> node_to_edge_;
