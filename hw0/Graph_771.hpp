...
class Graph {
 ...
 public:
  ...
  class Node {
   public:
    ...
    const Point& position() const {
      // HW0: YOUR CODE HERE
        return graph_->mapIndexToPoint[node_id_];
    }
  ...
  Node add_node(const Point& position) {
    ...
    mapIndexToNode[size_] = node;
    mapIndexToPoint[size_] = position;
    nodes.insert(node);
    ++size_;
    return node;
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (!has_edge(a, b)) {
     edges[a].insert(b);
     edges[b].insert(a);
     mapIndexToEdge[num_edges_] = edge;
     ++num_edges_;
    }
    return edge;
  }
 ...
 private:
  ...
  //   helper functions, data members, and so forth.
    /*
     Node attributes
     */
    std::map<size_type, Point> mapIndexToPoint;
    std::map<size_type, Node> mapIndexToNode;
    std::set<Node> nodes;
    size_type size_;
    /*
     Edge attributes
    ...
    std::map<size_type, Edge> mapIndexToEdge;
