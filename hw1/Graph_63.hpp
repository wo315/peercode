...
class Graph {
 ...
 private:
    ...
    //store 2 data points for every edge
 ...
 public:
  ...
  Graph()
    ...
    nodes_neighbour(), nodes_val(),nodes_adjacent() {}
  ...
  class Node: private totally_ordered<Node>{
   ...
   public:
    ...
    const Point& position() const {
      ...
      //return graph_->internal_nodes.at(uid_)
      assert(false);
    }
    ...
    incident_iterator  edge_end() const {return IncidentIterator(graph_,uid_,graph_->nodes_neighbour[uid_].size());}
    // HW1: YOUR CODE HERE
  ...
  class Edge: private totally_ordered<Edge> {
   public:
    ...
    Edge(const Graph* g,size_type n1, size_type n2)
      : graph_(const_cast<Graph*>(g)),uid1_(n1), uid2_(n2){}
    ...
    bool operator==(const Edge& e) const {
      bool res = (uid1_ == e.uid1_ && uid2_ == e.uid2_) ? true:false;
      return res;
    }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    if (nodes_neighbour.find(n1)== nodes_neighbour.end())
      return false; //not found
    else { //found
      auto v= nodes_neighbour.at(n1);
      for (auto edges_uid:v) //all these are edges uid
        if (internal_edges.at(edges_uid)[1]==n2)
          return true;
      return false;
    } 

    /*
    // std::unordered_map<size_type, std::array<size_type,2>>:: iterator itr; 
    for (auto itr = internal_edges.begin(); itr != internal_edges.end(); itr++) 
    { 
        if((itr->second)[0]== n1 && (itr->second)[1]== n2)
          return true;
     } 
    return false; */
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (has_edge(a,b))
      return Edge(this,a.index(),b.index()); // found an existing edge, will return it
    ...
    edges_uid_++;
    edges_size_++;
    return Edge(this,n1,n2);        // Invalid Edge
  }
  ...
  class IncidentIterator {
   public:
    ...
    Edge operator*() const {return Edge(graph_,uid_,graph_->nodes_neighbour[uid_][curr_ind]);}
    IncidentIterator& operator++() {
      curr_ind++;
      return *this;
    }
