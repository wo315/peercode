 ...
 public:
  ...
  class Edge {
   public:
    ...
    Node node1() const {
      // HW0: YOUR CODE HERE
      return Node(g__, uid_1);      // Invalid Node
    }

    /** Return the other node of this Edge */
    Node node2() const {
      // HW0: YOUR CODE HERE
      return Node(g__, uid_2);      // Invalid Node
    }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    if (has_node(a) && has_node(b)) {
      for (size_type j = 0; j<num_edges(); ++j) {
        if ((a.index() == edge_node1_[j] && b.index() == edge_node2_[j]) 
             || (b.index() == edge_node1_[j] && a.index() == edge_node2_[j])) {
          return true;
        }
      }
    }
    ...
    if (has_node(a) && has_node(b) && !(a==b)) {
      if (!(has_edge(a, b)) && !(has_edge(b, a))) {
        ...
        for (size_type k = 0; k<num_edges(); ++k) {
          if ((a.index() == edge_node1_[k] && b.index() == edge_node2_[k]) 
               || (b.index() == edge_node1_[k] && a.index() == edge_node2_[k])) {
