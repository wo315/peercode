...
class Graph {
 private:
  ...
  // HW0: YOUR CODE HERE
  std::vector<Point> point_list_; // [point1, point2, ...]
  std::vector<std::tuple<unsigned int, unsigned int>> edges_list_; // [(0, 1), (0, 2), ...] point indices
 public:
  ...
  class Node {
   public:
    ...
    const Point& position() const {
      // HW0: YOUR CODE HERE
      return graph_ptr_->point_list_[indx_];
    }
   ...
   private:
    ...
    friend class Graph;
    // HW0: YOUR CODE HERE
    size_type indx_; // index
    const graph_type* graph_ptr_; // graph
    Node(size_type indx, const graph_type* graph_ptr)
      : indx_(indx), graph_ptr_(graph_ptr) {
      }
  };
  ...
  Node add_node(const Point& position) {
    // HW0: YOUR CODE
    this->point_list_.emplace_back(position);
    Node new_node = Node(size()-1, this);
    return new_node;
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    // HW0: YOUR CODE HERE
    bool has = false;
    unsigned int point1_indx = (unsigned int)a.indx_;
    unsigned int point2_indx = (unsigned int)b.indx_;
    for (size_type i=0; i < num_edges(); i++) {
      if (std::get<0>(edges_list_[i]) == point1_indx) {
        if (std::get<1>(edges_list_[i]) == point2_indx) {
          has = true;
          break;
        }
      }
      else if (std::get<0>(edges_list_[i]) == point2_indx) {
        if (std::get<1>(edges_list_[i]) == point1_indx) {
          has = true;
          break;
        }
      }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (not has_edge(a, b)) {
      ...
      return Edge(edges_list_.size()-1, this); // new edge with index of the pair and the current graph
    }
    else {
