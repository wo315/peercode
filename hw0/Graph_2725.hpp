   ...
   private:
    ...
    Node(const Graph* graph, size_type idx)
      : graph_(const_cast<Graph*>(graph)), idx_(idx) {
        graph_->node_vector.push_back(*this);
        }
  ...
  class Edge {
   ...
   private:
    ...
    Edge(Graph* graph, size_type idx1, size_type idx2, size_type edge_idx)
      ...
      edge_idx_(edge_idx) {
        if(idx1<idx2) {
          graph_->node_to_edge_map[{idx1,idx2}]=edge_idx;
          graph_->edge_vector.push_back(*this);
        }
        if(idx1>idx2) {
          graph_->node_to_edge_map[{idx2,idx1}]=edge_idx;
          graph_->edge_vector.push_back(Edge());
          // cannot call the other constructor because it flips idx1,idx2
          // node_to_edge_map,
          // so we create an invalid node 
          // and manually set this internal edge to
          // have idx1<idx2, but the node given to client has order that
          // they requested
          graph_->edge_vector[edge_idx].graph_=graph;
          graph_->edge_vector[edge_idx].idx1_=idx2;
          graph_->edge_vector[edge_idx].idx2_=idx1;
          graph_->edge_vector[edge_idx].edge_idx_=edge_idx;
        }
      }
...
//        std::cout << node_to_edge_map.at({a.idx_,b.idx_}) << std::endl;
    ...
    // We call the constructor that stores the
    // new edge internally. Note if client asks for an edge with node1>node2,
    // the constructor will give them a node that matches that, but the
    // internal stored edge will always have node1<node2 to avoid duplication
    else {
      ++edge_size_;
      return Edge(this, a.idx_, b.idx_, edge_size_-1); 
    }
  }
