...
class Graph {
 private:
  ...
  std::vector <Point> nodes_; //vector of points/nodes
  std::map <unsigned int, std::pair<unsigned int, unsigned int>> edges_; //maps of edges with indexes as keys
  std::map <unsigned int, std::map<unsigned int, unsigned int>> n2e_; //maps of nodes to connected nodes and edge index
 ...
 public:
  ...
  class Node {
   public:
    ...
    const Point& position() const {
      // HW0: YOUR CODE HERE
      return this->graph_->nodes_[nid_];
    }
  ...
  class Edge {
   public:
    ...
    bool operator==(const Edge& e) const {
      ...
      if (graph_== e.graph_ and n1id_==e.n1id_ and n2id_==e.n2id_ and eid_==e.eid_){
        return true;
      }else{
        return false;
      }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (has_edge(a, b)){
      ...
      if(n2e_.count(ida) == 0) {
        std::map<unsigned int, unsigned int> nidmap1 = {{idb, eidx}};
        n2e_.insert({ida,nidmap1});
      }
      if(n2e_.count(idb) == 0){
        std::map<unsigned int, unsigned int> nidmap2 = {{ida, eidx}};
