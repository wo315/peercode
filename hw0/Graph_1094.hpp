...
class Graph {
 ...
 public:
  ...
  Graph() : nodes_(), num_nodes_(0), edges_(), edges_val(), num_edges_(0) {}
  ...
  class Node {
   public:
    ...
    const Point& position() const {
      return graph_->nodes_.at(n_idx_);
    }
   ...
   private:
    ...
    /** Construct a valid node by the given @a graph and @a idx.
     * @param[in] graph A pointer to the given Graph
     *            idx A unique index for the node
     */
  ...
  Node add_node(const Point& position) {
    ...
    nodes_.push_back(const_cast<Point&>(position));
    num_nodes_++;
    return new_node;
  }
