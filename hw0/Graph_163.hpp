...
// Declaring size_type before private attributes
 ...
 private:
  ...
  std::map<size_type, Point> node_map;
  //std::map<size_type, std::pair<size_type,size_type>> edge_map;
  std::vector<std::map<size_type, size_type>> edge_vec;
 ...
 //using size_type = unsigned;
  ...
  class Edge {
   public:
    ...
    Graph::Node node2() const {
      // HW0: YOUR CODE HERE
     // size_type n_ind = graph->edge_map[edge_id].second;
  ...
  Edge edge(size_type i) const {
    ...
    for (size_type j=0; j < num_nodes(); j++){
      if (edge_vec[j].count(i)){
        return Edge(this, i, std::make_pair(j, edge_vec[j].at(i)));
      }
    }
    return Edge(); // Invalid edge

    // For implementation when keys are pairs:

    // for (const auto e : edge_map){
    //     if(e.second == i) {
    //       return Edge(this, i, e.first);
    //     }
    // }
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    auto node_pair = create_node_pair(a_ind, b_ind);
    
    for (const auto& p: edge_vec[node_pair.first]){
      if (p.second == node_pair.second){
        return true;
      }
    }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    // This assures that first element in pair is always less than second
    // element, validating how you can compare pairs.

    auto node_pair = create_node_pair(a.index(), b.index());


    if (has_edge(a, b)){
      for (const auto& p: edge_vec[node_pair.first]){
        if (p.second == node_pair.second){
          return Edge(this, p.first, node_pair);;
        }
      }
    }

    Graph::size_type new_ind = num_edges();
    edge_vec[node_pair.first][new_ind] = node_pair.second; 
    //edge_map[node_pair] = new_ind;

    return Edge(this, new_ind, node_pair);        // Invalid Edge
  }
