...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position) {
    ...
    const node_value_type myvalue= node_value_type() ;
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    for (incident_iterator iter  = a.edge_begin(); iter !=a.edge_end(); iter++)
    {
      if(iter->node2() == b)
        return true;
    }
   ...
   public:
    ...
    bool operator==(const node_iterator& other_iter) const { 
       if (graph_ == other_iter.graph_)
         if (curr_node_index_ == other_iter.curr_node_index_)//if index of nodes are equal
           return true;
       return false;
     }
