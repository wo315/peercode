 ...
 public:
  ...
  class Edge {
   ...
   public:
    ...
    Node node1() const {
      // HW0: YOUR CODE HERE
      return Node(graph_, fetchendpoints().first);
    }

    /** Return the other node of this Edge */
    Node node2() const {
      // HW0: YOUR CODE HERE
      return Node(graph_, fetchendpoints().second);
    }
   ...
   private:
    ...
    // Private Constructor
    Edge(const Graph* newgraph, size_type new_edge_id)
      : graph_(const_cast<Graph*>(newgraph)), edge_id_(new_edge_id) {
    } 

    // Helper method to return the correct points
    // associated with the Edge
    PointPair fetchendpoints() const {
      return graph_->endpoints_.at(edge_id_);
    };
