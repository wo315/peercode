...
class Graph {
 ...
 public:
  ...
  Graph() : points_(), v_(), num_nodes_(), num_edges_(), edge_to_nodes_(),
            node_to_node_to_edge_(), ev_(), node_i2u_(), node_u2i_(),
            edge_i2u_(), edge_u2i_() {
    // HW0: YOUR CODE HERE
  }
  ...
  size_type remove_node(const Node& n) {
    ...
    if(has_node(n)) {
      ...
      node_u2i_[back_node_uid] = node_u2i_[node_uid];
      num_nodes_ = node_i2u_.size();
      ret_val = 1;
    }
    return ret_val;
  ...
  size_type remove_edge(const Node& a, const Node& b) {
    ...
    if(has_edge(a,b) and has_edge(b,a)){
      ...
      edge_u2i_[back_edge_uid] = edge_u2i_[edge_uid];
      num_edges_ -= 1;
      ret_val = 1;
    }
    return ret_val;
  ...
  class NodeIterator : private totally_ordered <NodeIterator> { 
   ...
   private:
    ...
    // Private constructor
    NodeIterator(const Graph* gra,
                 std::vector<size_type>::const_iterator node_it,
