...
class Graph {
 ...
 public:
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if (edge_index > -1) {
      // Edge already exists
      return Edge(this, size_type(edge_index)); 
    }
 ...
 private:
  ...
  struct NodeData {
    Point position;   
    NodeData(Point position) : position(position) {}
  };

  struct EdgeData {
    Node node1;
    Node node2;
    EdgeData(Node node1, Node node2) : node1(node1), node2(node2) {}
  };
  ...
  /** Return the index of the edge connecting two nodes.
   * @pre @a a and @a b are valid nodes of this graph
  ...
  int index(const Node& a, const Node& b) const {
    ...
    if (search1 != edge_map_.end()) {
      // found
      return search1->second;
    } else if (search2 != edge_map_.end()) {
      // found
