...
class Graph {
 ...
 public:
  ...
  class Node {
   public:
    ...
    size_type index() const {
      // HW0: YOUR CODE HERE
      assert(this->graph_ != NULL);
      assert(graph_->size() >= 0 and node_idx_ < graph_->size());
  ...
  Node add_node(const Point& position) {
    ...
    nodeVector.emplace_back(num_nodes_, position, adjEV);
 ...
 private:
  ...
  struct nodeProxy {
    ...
    nodeProxy (size_type ui, Point P1, std::vector<adjEdges> adjEV1) 
              :i(ui), P(P1), adjEV(adjEV1) {}
