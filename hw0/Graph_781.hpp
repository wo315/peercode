...
class Graph {
 ...
 public:
  ...
  Graph() 
    : next_uid_(0), nodes_(), edges_(), edges_set_() {
  }
   ...
   public:
    ...
    bool operator<(const Edge& e) const {
      ...
      return ( (graph_ < e.graph_) ||
        ( 
          (graph_ == e.graph_) && 
          ( (this->node1().index() < e.node1().index()) 
            || ( (this->node1().index() == e.node1().index()) && (this->node2().index() < e.node2().index()) ) )
        )
      );
  ...
  bool has_edge(const Node& a, const Node& b) const {
    // HW0: YOUR CODE HERE
    Edge edge(this, &a, &b);
    if(b.node_index_ < a.node_index_){  // Node1 is always the node with the smallest index
      edge.node1_ = &b;
      edge.node2_ = &a;
    }

    std::unordered_set<Edge, EdgeHash>::const_iterator got = edges_set_.find(edge);
    return ( got != edges_set_.end() ); 
