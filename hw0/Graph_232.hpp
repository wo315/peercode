...
class Graph {
 ...
 public:
  ...
  Graph() {
    // HW0: YOUR CODE HERE
    node_list = std::vector<internal_node>();
  ...
  class Node {
   public:
    ...
    size_type index() const {
      // for (size_type i = 0; i < graph_->node_list.size(); ++i)
      //   if (graph_->node_list.at(i).uid == uid_)
      //     return i;
      return index_;
      assert(false);
      // HW0: YOUR CODE HERE
    ...
    bool operator==(const Node& n) const {
      ...
      if (graph_ == n.graph_ && index_ == n.index_) return true;
      return false;
    }
   ...
   private:
    ...
    internal_node& fetch() const {
      return const_cast<internal_node&>(graph_->node_list.at(index_));
      assert(false);
    }
  ...
  Node add_node(const Point& position) {
    ...
    internal_node new_node = internal_node(position, node_uid);
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    (void) a, (void) b;   // Quiet compiler warning
    for (auto & element : edge_adjacency_list.at(a.index())) {
      if (element.a == b.index() || element.b == b.index()) return Edge(this, element.index);
    }
    ...
    internal_edge e = internal_edge(a_ind, b_ind, i);
    
    edge_adjacency_list.at(a_ind).push_back(e);
    edge_adjacency_list.at(b_ind).push_back(e);
