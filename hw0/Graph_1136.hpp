...
class Graph {
 ...
 public:
  ...
  // Adjacency list (IMPORTANT: for an edge (a,b), the node max(a,b) will be found 
  // in the vector adjacency_list[min(a,b)], there is no redundancies)
  std::vector<std::vector<size_type>> adjacency_list;

  // Map indices to edges (useful to accelerate edge(i))
  std::map<size_type, Edge> edges;
  ...
  Graph(){
    // HW0: YOUR CODE HERE
    this->nodes = std::vector<const Point*>(0);
    this->adjacency_list = std::vector<std::vector<size_type>>(0);
    this->edges = std::map<size_type, Edge>();
    this->num_edge = 0;
  }
  ...
  bool has_edge(const Node& a, const Node& b) const {
    ...
    size_type min_index = std::min(a.node_index, b.node_index);
    size_type max_index = std::max(a.node_index, b.node_index);

    for (size_type i = 0; i < this->adjacency_list[min_index].size(); i++)
    {
      if (this->adjacency_list[min_index][i] == max_index) {return true;}
    }
    return false;
  }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if(this->has_edge(a, b)){return Edge(this, a.node_index, b.node_index);}
      ...
      this->adjacency_list[min_index].push_back(max_index);
      // Increment the number of edges
      this->num_edge ++;
      // Update the list of edges
      this->edges.insert({this->num_edge - 1, Edge(this, a.node_index, b.node_index)});
