...
class Graph {
 ...
 public:
  ...
  class Node : private totally_ordered<Node>{
    ...
    */
    incident_iterator edge_begin() const {
      return IncidentIterator( graph_, index_, graph_->edges.at(index_).cbegin() );
    }
    ...
    */
    incident_iterator edge_end() const {
      return IncidentIterator( graph_, index_, graph_->edges.at(index_).cend() );
    }
  ...
  class Edge : private totally_ordered<Node>{
   public:
    ...
    Node node1() const {
      // HW0: YOUR CODE HEREN
      return graph_->node( std::get<0>( graph_->edge_id.at( index_ ) ) );      
    }

    /** Return the other node of this Edge */
    Node node2() const {
      // HW0: YOUR CODE HERE
      return graph_->node( std::get<1>( graph_->edge_id.at( index_ ) ) ); ;     
    }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if( has_edge( a, b ) ) {
      ...
      if( edge(eid).node1().index() == y ) edge_id[eid] = std::make_pair(x, y); 
  ...
  class IncidentIterator {
    ...
    */
    Edge operator*() const {
      ...
      if( e.node1().index() != rootid) graph_->edge_id[id] = std::make_pair( e.node2().index(), e.node1().index() );
      return e;
    }
