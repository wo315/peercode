 ...
 public:
  ...
  class Edge {
   public:
    ...
    Node node1() const {
      // HW0: YOUR CODE HEREN
      return graph_->node( std::get<0>( graph_->edge_id.at( index_ ) ) );      // Invalid Node
    }

    /** Return the other node of this Edge */
    Node node2() const {
      // HW0: YOUR CODE HERE
      return graph_->node( std::get<1>( graph_->edge_id.at( index_ ) ) ); ;      // Invalid Node
    }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    // HW0: YOUR CODE HERE
    (void) a, (void) b;   // Quiet compiler warn
    size_type x, y;
    x = a.index_;
    y = b.index_;
    
    if( has_edge( a, b ) ) {
      return Edge( this, edges.at(x).at(y) );
