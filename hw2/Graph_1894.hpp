...
class Graph {
   ...
   public:
    ...
    class Node : private totally_ordered<Node> {
       public:
        ...
        Point& position() { return graph_->node_uid_map_.at(uid_).point; }

        /** Return this node's index, a number in the range [0, graph_size). */
        size_type index() const {
            for (size_type i = 0; i < graph_->size(); i++) {
                if (graph_->node_vec_.at(i) == uid_) {
                    return i;
                }
            }
            return graph_->size();
       ...
       private:
        ...
        internal_node get_by_uid(node_id_type uid) const {
            return graph_->node_uid_map.at(uid);
        }
    };
    ...
    Node add_node(const Point& position,
        ...
        node_vec_.push_back(next_uid_);
        node_uid_map_.insert({next_uid_, node});
        node_to_edges_.insert({next_uid_, std::unordered_set<edge_id_type>()});
        next_uid_++;
    ...
    Node node(const size_type i) const {
        node_id_type uid = node_vec_.at(i);
    ...
    class Edge : private totally_ordered<Edge> {
       public:
        ...
        edge_value_type& value() {
            return graph_->edge_uid_map_.at(edge_uid_).value;
        }
        ...
        bool operator==(const Edge& e) const {
            ...
            return (m_one_ == e.m_one_ && m_two_ == e.m_two_) ||
                   (m_one_ == e.m_two_ && m_two_ == e.m_one_);
        }
    ...
    Edge edge(const size_type i) const {
        const edge_id_type& uid = edge_vec_.at(i);
        const internal_edge& edge = edge_uid_map_.at(uid);
    ...
    bool has_edge(const Node& a, const Node& b) const {
        const internal_edge* ptr = _maybe_get_internal_edge_ptr(a, b);
        return ptr != nullptr;
    }
    ...
    Edge add_edge(const Node& a, const Node& b) {
        const internal_edge* ptr = _maybe_get_internal_edge_ptr(a, b);
        if (ptr != nullptr) {
            internal_edge edge = *ptr;
            return Edge(this, edge.uid, a.uid_, b.uid_);
        }
        ...
        internal_edge edge = internal_edge(edge_uid, a.uid_, b.uid_);
        edge_uid_map_.insert({edge_uid, edge});

        // add the edge to BOTH node lists
        node_to_edges_.at(a.uid_).insert(edge_uid);
        node_to_edges_.at(b.uid_).insert(edge_uid);
    ...
    class IncidentIterator : private totally_ordered<IncidentIterator> {
       public:
        ...
        IncidentIterator() : m_graph(nullptr), m_node(-1), m_it() {}
         ...
         */
        Edge operator*() const {
            const edge_id_type& edge_id = *m_it;
            const internal_edge& edge = m_graph->edge_uid_map_.at(edge_id);
            const node_id_type& other =
                m_node == edge.one ? edge.two : edge.one;
       ...
       private:
        ...
        node_id_type m_node;
        std::unordered_set<edge_id_type>::iterator m_it;

        /**
         * An `IncidentIterator` essentially wraps an iterator
         * to the underlying unordered set.
         *
         * @param graph The associated graph
         * @param node The unique id of the node we will be iterating through
         * @param begin Since the underlying data structure is an unordered_set,
         *              we can only create iterators to the beginning or the
         * end.
         */
        IncidentIterator(const Graph* graph, const node_id_type node,
                         const bool begin)
            : m_graph(const_cast<Graph*>(graph)), m_node(node) {
            std::unordered_set<edge_id_type>& set =
                m_graph->node_to_edges_.at(m_node);
            if (begin) {
                m_it = set.begin();
            } else {
                m_it = set.end();
    ...
    class EdgeIterator : private totally_ordered<EdgeIterator> {
       public:
        ...
        Edge operator*() const {
            edge_id_type& id = m_graph->edge_vec_.at(m_k);
            internal_edge& edge = m_graph->edge_uid_map_.at(id);
            return Edge(m_graph, edge.uid, edge.one, edge.two);
        }
        ...
        bool operator==(const EdgeIterator& other) const {
            return m_graph == other.m_graph && m_k == other.m_k;
        }
       ...
       private:
        ...
        EdgeIterator(const Graph* graph, size_type k)
            : m_graph(const_cast<Graph* const>(graph)), m_k(k) {}
    };

    edge_iterator edge_begin() const { return EdgeIterator(this, 0); }
    edge_iterator edge_end() const { return EdgeIterator(this, num_edges()); }

    /** Remove a node and the associated edges from the graph */
    size_type remove_node(const Node& node) {
        // pass through incident edges and collect adjacent nodes
        std::unordered_map<edge_id_type, node_id_type> adjacent;
        for (auto it = node.edge_begin(); it != node.edge_end(); ++it) {
            Edge edge = *it;
            Node other = edge.node2();
            assert(other != node);
            adjacent.insert({edge.edge_uid_, other.uid_});
        }

        // we have now collected the uids of all adjacent nodes and
        // the connecting edges. We next disconnect the edge from the
        // other node by erasing it from node_to_edges_
        for (auto it = adjacent.begin(); it != adjacent.end(); ++it) {
            const std::pair<edge_id_type, node_id_type>& pair = *it;
            const edge_id_type& edge_id = pair.first;
            const node_id_type& node_id = pair.second;
            node_to_edges_.at(node_id).erase(edge_id);

            // we also have to clean up the edge
            edge_uid_map_.erase(edge_id);
            edge_vec_.erase(_edge_iterator_to(edge_id));
        }

        // Now we have to clean up for the requested node.
        // First, remove node from vector O(n)
        size_type index = node.index();
        if (index == size()) {
            return 0;
        }
        node_vec_.erase(node_vec_.begin() + index);

        // remove node from first map O(1)
        node_uid_map_.erase(node.uid_);

        // remove node from other map O(1)
        node_to_edges_.erase(node.uid_);
        return 1;
    }
    ...
    node_iterator remove_node(node_iterator n_it) {
        ...
        size_type removed = remove_node(node);
        if (removed == 1) {
            return node_begin();
        } else {
            return node_end();
    ...
    size_type remove_edge(const Node& a, const Node& b) {
        ...
        const internal_edge edge = *ptr;
        edge_vec_.erase(_edge_iterator_to(edge.uid));
        edge_uid_map_.erase(edge.uid);

        // We also need to remove the edge's from the node
        // adjacenecy lists
        node_to_edges_.at(a.uid_).erase(edge.uid);
        node_to_edges_.at(b.uid_).erase(edge.uid);
        return 1;
    }

    /** Remove an edge from the graph */
    size_type remove_edge(const Edge& edge) {
        const Node& a = edge.node1();
        const Node& b = edge.node2();
    ...
    edge_iterator remove_edge(edge_iterator e_it) {
        Edge edge = *e_it;
        size_type removed = remove_edge(edge);
        if (removed == 1) {
            return edge_begin();
        } else {
            return edge_end();
   ...
   private:
    /**
     * @brief Utility function to create an iterator pointing
     * to an index in the graphs `edge_vec_` vector corresponding
     * to the provided edge id.
     *
     * @param id The id of the edge
     */
    std::vector<edge_id_type>::const_iterator _edge_iterator_to(
        const edge_id_type id) const {
        for (auto it = edge_vec_.begin(); it != edge_vec_.end(); ++it) {
            if (*it == id) {
                return it;
            }
        }
        return edge_vec_.end();
    }

    /**
     * @brief Private convenience function for retrieving the
     * internal edge between two nodes. If no edges exists, a
     * `nullptr` is returned.
     */
    const internal_edge* const _maybe_get_internal_edge_ptr(
        const Node& a, const Node& b) const {
        const std::unordered_set<edge_id_type>& edge_ids =
            node_to_edges_.at(a.uid_);
        for (const edge_id_type id : edge_ids) {
            const internal_edge& edge = edge_uid_map_.at(id);
            if (b.uid_ == edge.one && a.uid_ == edge.two) {
                return &edge;
            }
            if (b.uid_ == edge.two && a.uid_ == edge.one) {
                return &edge;
            }
        }
        return nullptr;
    ...
    struct internal_edge {
        ...
        internal_edge(const edge_id_type uid_, const node_id_type one_,
                      const node_id_type two_)
            : uid(uid_), one(one_), two(two_), value() {}
    };

    // uid numbers are used for both node and edge, so they are
    // unique across all objects of the graph
    size_type next_uid_;

    std::vector<node_id_type> node_vec_;
    ...
    std::unordered_map<node_id_type, std::unordered_set<edge_id_type>>
        node_to_edges_;
};
