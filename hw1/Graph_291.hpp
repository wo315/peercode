...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position, const node_value_type& nodeval = node_value_type()) {
    ...
    node_vector.emplace_back(num_nodes_, position, adj_nodes, nodeval);
  ...
  class Edge : private totally_ordered<Edge>{
   public:
     ...
     */
    bool operator<(const Edge& e) const {
      return ((eid_ < e.eid_) or (node1() < e.node1()));
    }
  ...
  Edge add_edge(const Node& a, const Node& b) {
    if (!has_edge(a, b)) {
      edge_vector.emplace_back(num_edges_, a.global_index_, b.global_index_);
 ...
 private:
  ...
  struct pedge {
    ...
    pedge(size_type eidx, size_type id1, size_type id2) 
      :eidx(eidx), id1(id1), id2(id2) {}
  };
