...
class Graph {
 ...
 public:
  ...
  Node add_node(const Point& position) {
    // HW0: YOUR CODE HERE
    std::vector<NeghProxy> NodeNeigh{};
    //adding node to node registration list
    node_list.emplace_back(node_list.size(),position,NodeNeigh);
  ...
  Edge add_edge(const Node& a, const Node& b) {
    ...
    if(!has_edge(a,b)){
      if(a<b){//adding node
        edge_list.emplace_back(a.node_index,b.node_index,edge_list.size());
        edge_num++;
        //register to neighborhood list
        node_list[a.node_index].neighborhood_list.emplace_back(b.node_index,edge_list.size()-1);
        node_list[b.node_index].neighborhood_list.emplace_back(a.node_index,edge_list.size()-1);
 ...
 private:
  ...
  struct NodeProxy{
    ...
    NodeProxy(size_type id_val, Point P_val,std::vector<NeghProxy> neighborhood_list_val):id_np(id_val),P(P_val),neighborhood_list(neighborhood_list_val){}
  ...
  struct EdgeProxy{
    ...
    EdgeProxy(size_type node1_id_val, size_type node2_id_val, size_type edge_id_val):node1_id(node1_id_val),node2_id(node2_id_val),edge_id(edge_id_val){}
  };
